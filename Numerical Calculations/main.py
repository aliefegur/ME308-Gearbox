...
def calculate_possible_helical_properties():
    ...
    for N in range(min_pinion_teeth, min_pinion_teeth + 20):
        for module in standard_modules['modules'].values:
            ...
            # Apply force
            pinion.apply_force((60000*POWER)/(np.pi * N * module * RPM))
            gear.apply_force(pinion.applied_force())
            ...
            element_data.append({
                ...
                'd_p': pinion.d,                # mm
                'd_g': gear.d,                  # mm
                'W_p': pinion.applied_force(),  # N
                'W_g': pinion.applied_force(),  # N
...
def calculate_possible_bevel_properties():
    ...
    e = 1 / 5       # Gear ratio
    POWER = 4       # kW
    RPM = 1350 / 7  # Input rpm of the gear couple
    ...
    for N in range(min_pinion_teeth, min_pinion_teeth + 20):
        for module in standard_modules['modules'].values:
            ...
            element_data.append({
                ...
                'd_p': pinion.d,
                'd_g': gear.d,
                'W_p': pinion.applied_force(),
                'W_g': gear.applied_force(),
...
helicals = df1[(df1['stress_p'] < 180) & (df1['d_p'] > 50) & (df1['d_g'] < 400) & (df1['N_p'] % 2 == 0) & (df1['N_g'] % 2 == 0)]
bevels = df2[(df2['stress_p'] < 180) & (df2['d_p'] > 50) & (df2['d_g'] < 400) & (df2['N_p'] % 2 == 0) & (df2['N_g'] % 2 == 0)]
